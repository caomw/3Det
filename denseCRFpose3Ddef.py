...
for it in range(cpit,cfg.posit):
    ...
    for idl,l in enumerate(lpdet):
        ...
        if cfg.usedef:
            for idp,p in enumerate(models[idm]["ww"]):
                scr-=models[idm]["ww"][idp].dfax*(lpdef[idl][idp][0]**2)
                scr-=models[idm]["ww"][idp].dfay*(lpdef[idl][idp][1]**2)
                scr-=models[idm]["ww"][idp].dfaz*(lpdef[idl][idp][2]**2)
...
##            #models[idm]=model.w2model3D(models[idm],w[:-1],-w[-1]*bias,cfg.usebiases,cfg.usedef)
        ...
        for idl,l in enumerate(lndet):
            ...
            if cfg.usedef:
                for idp,p in enumerate(models[idm]["ww"]):
                    scr-=models[idm]["ww"][idp].dfax*(lndef[idl][idp][0]**2)
                    scr-=models[idm]["ww"][idp].dfay*(lndef[idl][idp][1]**2)
                    scr-=models[idm]["ww"][idp].dfaz*(lndef[idl][idp][2]**2)
